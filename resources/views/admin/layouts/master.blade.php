...
{{-- <body class="hold-transition dark-skin sidebar-mini theme-primary fixed rtl"> --}}
    ...
    switch(type){
        case 'info':
            '.tst1'.info(" {{ Session::get('message') }} ");
            break;

        case 'success':
            '.tst1'.success(" {{ Session::get('message') }} ");
            break;

        case 'warning':
            '.tst1'.warning(" {{ Session::get('message') }} ");
            break;

        case 'error':
            '.tst1'.error(" {{ Session::get('message') }} ");
            break;
    }
    @endif
    </script> --}}
   ...
   <script src="{{URL::asset('assets/assets/vendor_components/jquery-toast-plugin-master/src/jquery.toast.js') }}"></script>
