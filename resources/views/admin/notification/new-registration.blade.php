                            ...
                            @isset($new_order_registeration)
                                @foreach ($new_order_registeration as $new_order)
                                    <tr class="Row{{ $new_order['id'] }}">
                                        <td><input type="checkbox" name="{{ $new_order['id'] }}" id="{{ $new_order['id'] }}"></td>
                                        <td hidden>{{ $new_order['id'] }}</td>
                                        <td class="w-80"><a><img class="avatar" src="../images/avatar/2.jpg"
                                            alt="..."></a></td>
                                        <td >
                                            {{-- admin.notification.get.user.info --}}
                                            <a onclick="get_user_info('{{route('admin.notification.get.user.info',$new_order['user']['id'])}}','{{ csrf_token() }}');" class="mailbox-name hover-primary"  data-toggle="modal" data-target="#modal-center">
                                                
                                                {{ $new_order['user']['id'] }} # {{ $new_order['user']['name'] }}</a></td>
