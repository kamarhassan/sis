   ...
   public function get_std_to_payment()
   {
      try {
         $std_registartion = StudentsRegistration::where('remaining', '>', 0)
            ...
            ->get();
//        return  $std_registartion[0]['cours']['categorty_grade_level']['grade']['grade'];
   ...
   public function import_std_excel(ImportStudentsFromExcelRequest $request)
   {
      try {
         ...
         $feerequired = CoursFee::where('cours_id', $cours_id)->get('id')->toArray();
         ...
         $fee_required = $this->cours_fee_repo->get_fee_required_cours($feerequired);
          ...
          *
          */
         $sponspr_ships_id = null;
         if ($request->sponsore_id) {

            $sponspr_ships = Sponsorship::create([
               'sponsor_id' => $request->sponsore_id,
               'cours_id' => $cours_id
            ]);
            $sponspr_ships_id = $sponspr_ships->id;
         }
         ...
         foreach ($user_data as $key => $value) {
            ...
            $std_register[] = [
               ...
               'sponsorship_id' => $sponspr_ships_id,
               'feesRequired' => json_encode($fee_required),
   ...
   public function students_profile($user_id)
   {
      //   return[ $user_id,
      //   $this->students->attendance_percent(8,1)];
      try {
//      User

         $std = $this->userrepo->get_user_by_id($user_id);
         ...
         if ($std) {
            ...
            foreach ($std->cours as $key => $cours) {
               ...
               $cours->tacher_ = $cours->teacher['name'];
               $cours->marks_percent = $this->students->marks_percent($cours->id, $cours->teacher_id, $user_id);

               $cours->attendance_percent = $this->students->attendance_percent($cours->id, $user_id);

            }
            //    $std->cours->each(function($cours,$user_id){

            // //   dd(  $cours->attendance_percent);

            //    return $cours;$user_id;
            //    });


            // return $std;

//            return C:\wamp64\www\sis\resources\views\admin\students\students-profile\std-information-and-profile.blade.php
            return view('admin.students.profile-and-cours.index', compact('std'));
         } else {
            toastr()->error(__('site.this students is not for you'));
