...
class StudentsRepository implements StudentsInterface
{
   ...
   public function students_with_marks($array_students_id)
   {
   }
   public function add_teams_account($arrayofinfoteams, $user_id)
   {
   }

   public function attendance_percent($cours_id, $user_id)
   {

      $attendance_info =  AttendanceInfo::where('cours_id', $cours_id)
         ->where('total_hours', '!=', 'null')->get('id')->toArray();

      if (count($attendance_info) > 0) {

         return  $attendance_details = AttendanceDetail::whereIn('attendance_info_id', array_map('current', $attendance_info))
         ->where('user_id',$user_id)
         ->where('total_hours', '!=', 'null')->get()->count();
         if ($attendance_details > 0) {

            return ($attendance_details / count($attendance_info)) * 100;
         }
         return;
      }
   ...
   public function marks_percent($cours_id, $teacher_id, $user_id)
   {

      // return $cours_id.','. $teacher_id.','. $user_id;
      $header_marks = HeaderMarks::where(['cours_id' => $cours_id, 'teacher_id' => $teacher_id])->first();
      if ($header_marks) {
         
         // dd($user_id);
           $mark = Marks::where('header_mark_id' ,$header_marks->id)->where('user_id',$user_id)->first();
