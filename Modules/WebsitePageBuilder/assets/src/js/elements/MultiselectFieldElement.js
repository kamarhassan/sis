class MultiselectFieldElement extends SuperElement {
    constructor(obj) {
        super(obj);
    ...
    name() {
        return getI18n('multiselect_field');
    }
    ...
    getControls() {
        ...
        return [
            ...
            new CommonFieldControl(getI18n('common_field'), {
                fieldName: element.obj.find('select').attr('name'),
                label: element.obj.find('label').html(),
                labelShowed: element.obj.find('label').is(':visible'),
                id: element.obj.find('select').attr('id'),
                icon: 'done_all',
            }, function(options) {
                element.obj.find('select').attr('name', options.fieldName);
                ...
                element.obj.find('select').attr('id', options.id);
            }),

            new PlaceholderControl(getI18n('placeholder'), {placeholder: element.obj.find('select').attr('placeholder')}, function(options) {
                element.obj.find('select').attr('placeholder', options.placeholder);
            }),

            new SectionTitleControl(getI18n('field_options')),

            // options control
            new FieldOptionsControl(getI18n('options'), {
                options: element.obj.find('select option').map(function() {
                    return {
                ...
                element.obj.find('select').html('');
                options.options.forEach(function(option) {
                    element.obj.find('select').append('<option value="' + option.value + '">' +option.text+ '</option>');
                });
            }),

            new ValidationControl(getI18n('validation'), {
                required: element.obj.find('select').attr('required'),
                minLength: element.obj.find('select').attr('data-min-length') ? element.obj.find('select').attr('data-min-length') : 1,
                maxLength: element.obj.find('select').attr('data-max-length') ? element.obj.find('select').attr('data-max-length') : '',
                regexp: element.obj.find('select').attr('data-regexp') ? element.obj.find('select').attr('data-regexp') : ''
            }, function(options) {
                if (options.required) {
                    element.obj.find('select').prop('required', true);
                } else {
                    element.obj.find('select').removeAttr('required');
                }

                //
                element.obj.find('select').attr('data-min-length', options.minLength);
                element.obj.find('select').attr('data-max-length', options.maxLength);
                element.obj.find('select').attr('data-regexp', options.regexp);
            }),
