class CheckboxFieldElement extends SuperElement {
    constructor(obj) {
        super(obj);
    ...
    getControls() {
        ...
        return [
            ...
            new FieldOptionsControl(getI18n('options'), {
                ...
                options: element.obj.find('.checkbox-option').map(function() {
                    return {
                ...
                var fname = element.obj.find('.inputs-group input').attr('name');
                element.obj.find('.inputs-group').html('');
                options.options.forEach(function(option) {
                    element.obj.find('.inputs-group').append(`
                        <span class="mr-4 checkbox-option d-block">
                            <input type="checkbox" name="`+fname+`" value="`+option.value+`" autocomplete="off">
