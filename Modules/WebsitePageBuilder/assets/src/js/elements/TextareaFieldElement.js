class TextareaFieldElement extends SuperElement {
    constructor(obj) {
        super(obj);
    ...
    getControls() {
        ...
        return [
            ...
            new CommonFieldControl(getI18n('common_field'), {
                fieldName: element.obj.find('textarea').attr('name'),
                label: element.obj.find('label').html(),
                labelShowed: element.obj.find('label').is(':visible'),
                id: element.obj.find('textarea').attr('id'),
                icon: 'article',
            }, function(options) {
                element.obj.find('textarea').attr('name', options.fieldName);
                ...
                element.obj.find('textarea').attr('id', options.id);
            }),

            new PlaceholderControl(getI18n('placeholder'), {placeholder: element.obj.find('textarea').attr('placeholder')}, function(options) {
                element.obj.find('textarea').attr('placeholder', options.placeholder);
            }),

            new ValidationControl(getI18n('validation'), {
                required: element.obj.find('textarea').attr('required'),
                minLength: element.obj.find('textarea').attr('data-min-length') ? element.obj.find('textarea').attr('data-min-length') : 1,
                maxLength: element.obj.find('textarea').attr('data-max-length') ? element.obj.find('textarea').attr('data-max-length') : '',
                regexp: element.obj.find('textarea').attr('data-regexp') ? element.obj.find('textarea').attr('data-regexp') : ''
            }, function(options) {
                if (options.required) {
                    element.obj.find('textarea').prop('required', true);
                } else {
                    element.obj.find('textarea').removeAttr('required');
                }

                //
                element.obj.find('textarea').attr('data-min-length', options.minLength);
                element.obj.find('textarea').attr('data-max-length', options.maxLength);
                element.obj.find('textarea').attr('data-regexp', options.regexp);
            }),
